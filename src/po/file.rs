...
use crate::{plural::PluralForms, comment::Comment, error::Error, note::Note, unit::Unit};
use crate::{Origin, State, CatalogueReader};
...
impl<'p, R: Read> PoFile<'p, R> {
    pub(super) fn new(reader: R, parser: &'p PoParser) -> Result<PoFile<'p, R>, Error> {
        let mut res = PoFile {
...
impl<'p, R: Read> Iterator for PoFile<'p, R> {
...
impl<'p, R: Read> CatalogueReader for PoFile<'p, R> {
...
mod tests {
    ...
    fn make_reader<R: Read>(reader: R, parser: &PoParser) -> PoFile<R> {
        let mut unit = Unit::default();
    ...
    fn test_empty() {
        ...
        let reader = PoFile::new(&empty[..], &parser).unwrap();
...
Expected one of r#"[0-9]+"#, "(", "-" or "n""##,
    ...
    fn test_func_new_with_error_on_entry() {
        let source = "msgid: \"--";
        let parser = PoParser::new();
        let reader = PoFile::new(source.as_bytes(), &parser);
    ...
    fn test_func_new_normal() {
        ...
        match PoFile::new(source.as_bytes(), &parser) {
            Ok(reader) => {
